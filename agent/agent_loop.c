...
************************macro define************************
***********************************************************/
#ifndef TCLAW_AGENT_STACK
#define TCLAW_AGENT_STACK   (24*1024)
#endif

#ifndef TCLAW_CONTEXT_BUF_SIZE
#define TCLAW_CONTEXT_BUF_SIZE   (32 * 1024)
#endif

#ifndef TCLAW_HISTORY_MAX_COUNT
#define TCLAW_HISTORY_MAX_COUNT  10
#endif
 ...
 * agent_loop_set_last_response() called from tclaw_chat.c on STREAM_STOP.
 ...
 * tclaw_chat.c uses this to suppress mid-stream overflow flushes. */
...
OPERATE_RET build_current_context(const char *role, const char *content)
{
    ...
    if (count >= TCLAW_HISTORY_MAX_COUNT) {
 ...
 *
 * Used by tclaw_chat.c to suppress intermediate IM flushes.
 */
 ...
 *
 * Called by tclaw_chat.c on AI_USER_EVT_TEXT_STREAM_STOP before
...
/**
 * agent_loop_notify_turn_done - Called by tclaw_chat.c on STREAM_STOP.
 *
...
{
    memset(s_total_prompt, 0, TCLAW_CONTEXT_BUF_SIZE);
    size_t off = 0;
    if (!is_tool) {
        /* First iteration: include full system prompt (tools, rules, memory, skills) */
        off = context_build_system_prompt(s_total_prompt, TCLAW_CONTEXT_BUF_SIZE);
        if (off == 0) {
    ...
    int count = cJSON_GetArraySize(s_history_json);
    if (count > 0) {
        off += snprintf(s_total_prompt + off, TCLAW_CONTEXT_BUF_SIZE - off,
    ...
    if (off + needed > TCLAW_CONTEXT_BUF_SIZE) {
...
static void agent_loop_task(void *arg)
{
    ...
    while (1) {
        ...
        while (iteration < TOOL_LOOP_MAX) {
            ...
            app_im_bot_send_message("TClaw is working...");
...
OPERATE_RET agent_loop_init_evt_cb(void *data)
{
    ...
    s_total_prompt = tal_psram_malloc(TCLAW_CONTEXT_BUF_SIZE);
#else
    s_total_prompt = tal_malloc(TCLAW_CONTEXT_BUF_SIZE);
#endif
    if (!s_total_prompt) {
        return OPRT_MALLOC_FAILED;
    }
    ...
    thrd_param.stackDepth = TCLAW_AGENT_STACK;
