 ...
 * @brief MCP tools registration for TClaw
 * @version 0.1
 * @date 2025-03-25
 *
...
static OPERATE_RET __ai_mcp_init(void *data)
{
    ...
    #if defined(PLATFORM_LINUX) && (PLATFORM_LINUX == 1)
        rt = ai_mcp_server_init("TClaw MCP Server", "1.0");
        if (rt != OPRT_OK) {
            return rt;
        }
    #endif
 ...
 * all custom tools for the TClaw project.
 *
...
OPERATE_RET tool_registry_init(void)
{
    ...
    PR_DEBUG("TClaw MCP tools registered successfully");
