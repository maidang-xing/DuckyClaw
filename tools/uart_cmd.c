 ...
 *
 * Reads available data from the UART port and appends it to sg_read_buffer.
 * Should be called before returning buffer contents to ensure fresh data.
 */
static void __uart_collect_data(void)
{
    ...
    while (sg_read_index < UART_CMD_BUF_SIZE - 1) {
        len = tal_uart_read(UART_CMD_PORT, (uint8_t *)tmp,
                            sizeof(tmp) < (UART_CMD_BUF_SIZE - 1 - sg_read_index)
                                ? sizeof(tmp) : (UART_CMD_BUF_SIZE - 1 - sg_read_index));
        if (len <= 0) {
            break;
        }
                                    ...
                                    MCP_RETURN_VALUE_T *ret_val, void *user_data)
{
    ...
    PR_DEBUG("uart_exec: sent '%s' (%d bytes), waiting %dms", command, write_len, wait_ms);

    /* Wait for the computer to execute the command */
