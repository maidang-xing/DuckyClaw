...
TClaw is a hardware-oriented AI agent built on the TuyaOpen C SDK. It runs a Claw-style agent loop on edge devices (Tuya T5AI, ESP32, Raspberry Pi, Linux) that communicates with users via IM channels (Telegram, Discord, Feishu) and executes MCP-style tools on the device.

## Build Commands

TClaw builds as a TuyaOpen application. From the repo root:
...
# Build (from TuyaOpen directory, pointing to TClaw as the app)
cd TuyaOpen
...
- Synchronization: `tclaw_chat.c` receives streaming AI events and calls `agent_loop_notify_turn_done()` on `AI_USER_EVT_END`
